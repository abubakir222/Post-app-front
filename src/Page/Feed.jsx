...
const socket = io('https://post-app-backend-8el3.onrender.com/api', { reconnection: true, reconnectionAttempts: 5, reconnectionDelay: 1000 });
const API_URL = 'https://post-app-backend-8el3.onrender.com/api/post';
...
const Feed = () => {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        'https://post-app-backend-8el3.onrender.com/api/comment',
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://post-app-backend-8el3.onrender.com/api/comment/${commentId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUpdateComment = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `https://post-app-backend-8el3.onrender.com/api/comment/${editingCommentId}`,
      ...
      <div className="container mt-5">
        ...
        <div className="post-grid">
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: 300, justifyContent: 'center' }}>
              <div style={{ marginTop: 24, fontSize: 24, fontWeight: 500 }}>Postlar yo‘q</div>
            </div>
          ) : (
            [...posts]
