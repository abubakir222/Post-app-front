...
const API = 'https://post-app-backend-8el3.onrender.com/api/admin';
...
const AdminPage = () => {
  ...
  const handleStatUserClick = async (user, type) => {
    ...
    try {
      let url = "";
      if (type === "like") url = `https://post-app-backend-8el3.onrender.com/api/user/${user._id}/liked-posts`;
      if (type === "comment") url = `https://post-app-backend-8el3.onrender.com/api/user/${user._id}/commented-posts`;
      if (type === "post") url = `https://post-app-backend-8el3.onrender.com/api/user/${user._id}/my-posts`;
    ...
    <div className="admin-container">
      ...
      <Modal
        ...
        title={
          statUserModal
            ? statUserType === "like"
              ? `${statUserModal.user?.username} — Layk bosgan postlari`
              : statUserType === "comment"
                ? `${statUserModal.user?.username} — Komment qoldirgan postlari`
                : `${statUserModal.user?.username} — Joylagan postlari`
            : ""
        }
      >
            ...
            {statUserPosts
              ...
              statUserPosts
                ...
                .map(post =>
                  <li key={post._id}>
                    <b>{post.title}</b>
                    <div style={{ color: "#008cff" }}>Like bosgan</div>
              ...
              statUserPosts
                ...
                .map(item =>
                  <li key={item.comment._id}>
                    <b>{item.post.title}</b>
                    <div style={{ color: "#008cff" }}>Comment yozgan:</div>
      ...
      {loading ? <Loader /> : (
        <>
          <ul className="admin-tabs">
            <li>
              <button className={tab === 'posts' ? 'active' : ''} onClick={() => setTab('posts')}>Postlar</button>
            </li>
            <li>
              <button className={tab === 'comments' ? 'active' : ''} onClick={() => setTab('comments')}>Kommentlar</button>
            </li>
            <li>
